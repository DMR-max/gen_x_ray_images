...
class configuration:
    ...
    if version != "3":
        path_mod_1 = input("Please select the path where the images and model saves should be stored of the second model (lr = 5e-4 or img dim = 32)")
    if version != "3" and version != "2":
        path_mod_2 = input("Please select the path where the images and model saves should be stored of the third model (lr = 1e-5)")
    ...
    directory_of_output = path_baseline
...
class train:
    def __init__(self):
        ...
        self.model_64 = diffusers.UNet2DModel(
        ...
        self.model_32 = diffusers.UNet2DModel(
            # Resolution of the images
            sample_size=self.train_conf.img_dim,
            # Amount of input channels, 1 for greyscale images 
            in_channels=1,
            ...
            layers_per_block=2,
            # Number of output channels for each UNet block (for 64 x 64 and higher 2, lower 1 is recommended) 
            block_out_channels=(self.train_conf.img_dim * 2, self.train_conf.img_dim * 4, self.train_conf.img_dim * 8),  
            down_block_types=(
                # This block downsamples the input
                "DownBlock2D",  
                # This block has spatial self-attention for improved spatial performance
    ...
    def model_evaluation(self, epoch, pipeline):
        ...
        img_grid.save(str(self.train_conf.directory_of_output) +"/" + str(epoch) + ".png")
    ...
    def noise_creator(self):
        ...
        # Add noise to the image
        predicted_noise = self.model_64(image_with_noise, steps_needed).sample
        # Calculate the mean squared error loss
        calculate_MSEloss_function = F.mse_loss(predicted_noise, random_noise)

    def model_training(self):
        if self.train_conf.version == "1":
            lr_list = [1e-4, 5e-4, 1e-5]
            img_dim_list = [64]
        if self.train_conf.version == "2":
            lr_list = [1e-4]
            img_dim_list = [32, 64]
        if self.train_conf.version == "3":
            lr_list = [1e-4]
            img_dim_list = [64]
        for self.train_conf.lr in lr_list:
            if self.train_conf.lr == 1e-4 and self.train_conf.version == "1":
                self.train_conf.directory_of_output = self.train_conf.path_baseline
            if self.train_conf.lr == 5e-4 and self.train_conf.version == "1":
                self.train_conf.directory_of_output = self.train_conf.path_mod_1
            if self.train_conf.lr == 1e-5 and self.train_conf.version == "1":
                self.train_conf.directory_of_output = self.train_conf.path_mod_2
            for self.train_conf.img_dim in img_dim_list:
                if self.train_conf.img_dim == 32 and self.train_conf.version == "2":
                    self.model = self.model_32
                    self.train_conf.directory_of_output = self.train_conf.path_mod_1
                elif self.train_conf.img_dim == 64 and self.train_conf.version == "2":
                ...
                # Train the model
                for epoch in range(self.train_conf.total_epochs):
                    ...
                    # Save the model
                    huggingface_train_package.save(self.model.state_dict(), self.train_conf.directory_of_output + '/model_weights_stable_diff_epoch_' + str(epoch) + 'img_dim '+ str(self.train_conf.img_dim) + ' lr = ' + str(self.train_conf.lr) + '.pth')
